...
void descartar_azul() {
    ...
    // Valor de referencia para saber si hay pelota
    // 190 suele funcionar bien para objetos muy cercanos
    int umbral_prox = 200; 
    ...
    while (true) {
        ...
        // Rango solicitado: 190 a 250 grados.
        // Según la rueda de color VEX, el azul puro es 240, así que este margen cubre bien.
        bool esAzul = (hue >= 190 && hue <= 250);
        ...
        // CONDICIÓN DE PARADA: 
        // Si detectamos AZUL Y hay un objeto cerca, lo descartamos.
        if (esAzul && hayObjeto) {
            
            // 1. Secuencia de rechazo / expulsión
            // (Mantenemos tus valores de motores originales para expulsar)
            intake11W.move(127);
            frontmotor.move(75);
            bigroller.move(127);
            stage2.move(-127); 
            ...
            // SI NO es azul (es rojo o no hay nada)
            // SEGUIR MOVIENDO HACIA ADELANTE (Recolección normal)
            intake11W.move(-127);
            frontmotor.move(-75);
            bigroller.move(-127);
            stage2.move(127); 
...
// Drive Example
///
void autonomo24ROJOodom() {
// Avanzamos para alinear a el primer cargador
    chassis.pid_odom_set(27.1_in, 90, true);  // Slew will be enabled for this motion
    chassis.pid_wait();
...
chassis.pid_wait();
   ...
   // encender_rollos();
    ...
    chassis.pid_odom_set(5.5_in, 80);  // Slew will be enabled for this motion
    chassis.pid_wait();
    ...
    //Gira hacia la meta 
    chassis.pid_turn_set(-86_deg, 40);
    chassis.pid_wait();

    //Gira para acomodarse
chassis.pid_swing_set(ez::RIGHT_SWING, -90_deg, 40);
chassis.pid_wait();
    ...
    chassis.pid_odom_set(17.35_in, 80, true);
    chassis.pid_wait();
...
chassis.pid_swing_set(ez::RIGHT_SWING, 91_deg, 40);
chassis.pid_wait();
   ...
   // chassis.pid_wait();
    ...
    chassis.pid_odom_set(18.2_in, 50, true);
    chassis.pid_wait();
...
chassis.pid_swing_set(ez::RIGHT_SWING, -90_deg, 45);
chassis.pid_wait();
    ...
    lift1.extend();
    lift2.extend();
    chassis.pid_odom_set(16.75_in, 50, true);
    chassis.pid_wait();
